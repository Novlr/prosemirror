...
function mapThrough(map, pos) {
  ...
  for (var _i = 0; _i < map.moved.length; _i++) {
    var _range = map.moved[_i];
    var start = back ? _range.dest : _range.start;
    if (pos.cmp(start) >= 0 && _model.Pos.cmp(pos.path, pos.offset, start.path, start.offset + _range.size) <= 0) {
      var dest = back ? _range.start : _range.dest;
