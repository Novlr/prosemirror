    ...
    value: function map(remapping) {
      ...
      if (this.to) {
        if (this.to.cmp(this.from) == 0) {
          ...
          var _result = remapping.map(this.to, -1);
          to = _result.pos.max(from);
          if (!_result.deleted) allDeleted = false;
        }
      }
      if (this.pos) {
        if (from && this.pos.cmp(this.from) == 0) {
          ...
          var _result2 = remapping.map(this.pos, 1);
          pos = _result2.pos;
          if (!_result2.deleted) allDeleted = false;
        }
      }
