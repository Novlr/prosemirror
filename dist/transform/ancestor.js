    ...
    if (types.length) {
      var _ret = function () {
        ...
        for (var _i = types.length - 1; _i >= 0; _i--) {
          node = types[_i].create(attrs[_i], node || content);
...
_transform.Transform.prototype.lift = function (from) {
  ...
  for (var _d = 0, _pos = range.from;; _d++) {
    if (_pos.offset > 0) {
      this.split(_pos, depth - _d);
      var cut = range.from.path.length - depth,
          path = _pos.path.slice(0, cut).concat(_pos.path[cut] + 1);
  ...
  if (found.unwrap) {
    ...
    for (var _i3 = rangeNode.iter(range.from.offset, range.to.offset), child; child = _i3.next().value;) {
      size += child.size;
    }var _path = range.from.path.concat(range.from.offset);
    range = { from: new _model.Pos(_path, 0), to: new _model.Pos(_path, size) };
    ++depth;
  }
...
_transform.Transform.prototype.wrap = function (from, to, type, wrapAttrs) {
  ...
  if (inside.length) {
    ...
    }for (var _i4 = range.to.offset - 1 - range.from.offset; _i4 > 0; _i4--) {
      this.split(new _model.Pos(toInner, _i4), inside.length);
    }
  }
  return this;
};
