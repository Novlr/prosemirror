...
function replace(node, from, to, root, repl) {
  ...
  if (depth == root.length) {
    ...
    var pos = root[depth];
    var _result = replace(node.child(pos), from, to, root, repl, depth + 1);
    if (!_result) return null;
    return { doc: node.replace(pos, _result.doc), moved: _result.moved };
  }
}
...
function buildInserted(nodesLeft, source, start, end) {
  ...
  for (;; searchRight--) {
    var _node = nodesRight[searchRight],
        type = _node.type,
        matched = null;
    var outside = searchRight <= same;
    // Find the first node (searching from leaf to trunk) which can
    // contain the content to be inserted.
    for (var _i = searchLeft; _i >= 0; _i--) {
      var left = nodesLeft[_i];
      if (outside ? left.type.canContainContent(_node.type) : left.type == type) {
        matched = _i;
        break;
      }
    }
    if (matched != null) {
      if (!result) {
        result = nodesLeft[matched].copy(_node.content);
        searchLeft = matched - 1;
      } else {
        while (searchLeft >= matched) {
          var wrap = nodesLeft[searchLeft];
          var content = _model.Fragment.from(result);
          result = wrap.copy(searchLeft == matched ? content.append(_node.content) : content);
          searchLeft--;
        }
      }
      if (outside) break;
...
_transform.Transform.prototype.replace = function (from, to, source, start, end) {
  ...
  if (nodesBefore.length && (nodesAfter.length != nodesBefore.length || !nodesAfter.every(function (n, i) {
    ...
    var offset = _after$shorten.offset;var before = void 0;
    for (var node = docAfter.path(path), _i2 = 0;; _i2++) {
      if (_i2 == nodesBefore.length) {
