...
function registerTokens(tokens, name, type, info) {
  if (info.parse == "block") {
    ...
    throw new _error.AssertionError("Unrecognized markdown parsing spec: " + info);
  }
}
  ...
  if (!found) {
    (function () {
      ...
      schema.registry("configureMarkdown", function (name, f) {
        if (name == "init") {
          if (init) throw new _error.AssertionError("Two markdown parser initializers defined in schema");
          init = f;
        } else {
